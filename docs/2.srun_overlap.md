...
`nvtop` is a (h)top like task monitor for GPUs and accelerators. (https://github.com/Syllo/nvtop). We have the `nvtop` binary included in `KIR-utils` module. In order to use it, 

- Access the node where the GPU job is running with `srun --overlap --job-id JOBID --pty bash` as before
- `KIR-utils` module should be loaded by default. If not, run `module load KIR-utils` 
- Then run `nvtop`

![nvtop](./images/nvtop.png){width="500" .centered}
